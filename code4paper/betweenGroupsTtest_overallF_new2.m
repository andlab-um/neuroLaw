...
%% path
%path.root='F:\Results';
%path.data='F:\Results\firstLevel_neurolaw_concated_20230419_forDCM';
%path.roi='F:\Results\secondlevel\ROI_analysis\ROIs_threeNetworks';
path.root='Z:\experiment_data\jiamin_neurolaw\';
%path.data=[path.root,'Number-Data\fMRI data\firstLevel_neurolaw_concated_divide_Time_cat'];

path.data=[path.root,'firstLevel_neurolaw_concated_current_delay_overall'];
path.roi=[path.root,'myroi6'];
%% matlab batch
% Initialize SPM
...
subject_2nd_part=subject([1:23,54:56],1);
subject_3rd_part=subject([24:53],1); % delete 38,48,49
...
contrast_set={'overall'};
           
workdir=fullfile(path.root,'secondlevel','ROI_analysis','betweenGroup_ttest2_ROIanalysis_overall_202308_2');
if ~exist(workdir,'dir')
mkdir(workdir);
end

pValue_ID_fdr=zeros(1,1);
pvalue_status=zeros(1,1);
