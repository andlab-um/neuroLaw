...
data_file = glob.glob(os.path.join(BEHAVIOR_ROOT, '3_2*'))[0]
df = pd.read_excel(data_file, skipfooter=76, sheet_name='Sheet1')
# hard-coded, for some reason skip footer is not working
df = df.iloc[:56,:]
# st.write(df)
...
df_long['group_rewards'] = df_long['group'].astype(str) + '-' + df_long['rewards'].astype(str)

# st.write(df_long)


json_file = './json/fig3_sub1.json'
## presets
...
with setup_cols[1]:
    ...
    st.write("Note: be cautious when changing the figure size, the positions of the double x-axis were hard-coded")
...
asp = float(modified['fig_width'])/float(modified['fig_height'])
...
# NOTE: hard-coded here, could find the best way to do it
groups = pd.unique([get_cond(["SPP", "TPP"], o) for o in order])
plot.ax.text(.5*float(modified['fig_width'])/10 * asp, 
             -float(modified['fig_height'])/4 * asp, groups[0], size=modified['tick_font_size'])

plot.ax.text((1 - 4/10 * asp)*float(modified['fig_width']), 
             -float(modified['fig_height'])/4 * asp, groups[1], size=modified['tick_font_size'])

plot.ax.xaxis.labelpad = 2*modified['tick_font_size']
plot.ax.vlines([1.5], 0, -.18, 
               color='black', lw=0.8, clip_on=False, transform=plot.ax.get_xaxis_transform())

plot.set_axis_labels(x_var=modified['x_axis_label'], y_var=modified['y_axis_label'])
# plot.ax.set_xticks(minor=True)
plot.set_xticklabels([get_cond(rewards,label) for label in order], 
                     size=modified['tick_font_size'])
...
if annot:
    ...
    # st.write(hue_order)
    # pairs = [[[(o, ho) for ho in comb] for comb in list(combinations(hue_order, 2))] for o in order]
    pairs = []
    for o in order:
        group_reward_combinations = list(combinations(hue_order, 2))
        for combination in group_reward_combinations:
            pairs.append([(o, combination[0]), (o, combination[1])])
    # st.write(pairs)
    # st.write(pairs)
